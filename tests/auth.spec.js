...
describe("Authentication", () => {
  ...
  describe("auth.register", () => {
    ...
    it("password needs at least six characters", done => {
      api.actions
        ...
        .should.be.rejected()
        .then(_ =>
          api.actions.call("auth.register", {
    ...
    it("create a new user", done => {
      api.actions.call("auth.register", validUser).then(response => {
  ...
  describe("auth.login", () => {
    ...
    it("must require the email parameter", done => {
      api.actions
        .call("auth.login", { password: "something" })
        .should.be.rejected()
        .then(_ => {
          done();
        });
    ...
    it("must require the password parameter", done => {
      api.actions
        .call("auth.login", { email: "someuser" })
        .should.be.rejected()
        .then(_ => {
          done();
        });
    ...
    it("must generate a token", done => {
      api.actions.call("auth.login", validUser).then(response => {
    ...
    it("disable user can not make login", done => {
      api.actions
        .call("auth.login", disabledUser)
        .should.be.rejectedWith({ message: "The user are disable" })
        .then(_ => {
          done();
        });
    });
  ...
  describe("auth.checkSession", () => {
    ...
    it("returns an error when the token is invalid", done => {
      api.actions
        .call("auth.checkSession", { token: "invalid_token" })
        .should.be.rejectedWith({ code: "malformed_token" })
        .then(_ => {
          done();
        });
    });
    ...
    it("do not return an error when the token is valid", done => {
      ...
      // get a valid token
      api.actions
        .call("auth.login", validUser)
        .then(response =>
          api.actions.call("auth.checkSession", { token: response.token }),
        )
        .then(response => {
  ...
  it("can enable a disabled user account ", done => {
    api.actions
      .call("auth.activateUser", { id: disabledUserId })
